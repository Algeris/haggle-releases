...
You can find the current version in the app's **Settings → About** screen, in `package.json` at the repo root, or on the [GitHub Releases page](https://github.com/some1-ops/haggle-desktop/releases).
...
Email **contact@algeris.com** with a clear subject line such as "Security report — [short description]".
...
- **The haggle.software website** — server misconfigurations, common web vulnerabilities, and information leaks.
...
- **Prompt-injection attacks** that cause the underlying AI provider to behave in unintended ways. These are limitations of the underlying models, are in scope for the model provider's own safety processes, and are discussed in our [Terms &amp; Conditions §11](https://haggle.software/termsandconditions). Reports about a *specific way to bypass a safety filter on OpenAI / Anthropic / Google models* should go to the respective provider.
...
<https://github.com/some1-ops/haggle-desktop/security/advisories>
...
For Privacy Policy questions, see [PRIVACY.md](./PRIVACY.md). For commercial / refund / licensing questions, see the [Refund Policy](https://haggle.software/refundpolicy) and [Terms &amp; Conditions](https://haggle.software/termsandconditions).
